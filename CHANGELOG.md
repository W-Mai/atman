...
- **Final response convergence** — Managed root agents explicitly require a single `final.answer` call when a complete response is emitted as ordinary text, then accept the next classifier-approved response instead of repeating the delivery reminder indefinitely.
