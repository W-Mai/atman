...
- **Project Hub navigation** — The startup Recent Sessions list ends with a hoverable and clickable `All Projects [Ctrl+L]` record using the same four-row spacing, horizontal padding, and pointer target as recent sessions. The shortcut opens a full-screen project workspace with a two-column board, fixed inspector, constrained session list, persistent item and page counts, keyboard and pointer paging, and project-root-bound session switching. Project and session items use layered backgrounds with a full-height selection marker.
