...
- **Project Hub navigation** — The startup Recent Sessions list ends with a hoverable and clickable `All Projects [Ctrl+L]` row. The shortcut opens a full-screen project workspace with a two-column board, fixed inspector, constrained session list, persistent item and page counts, keyboard and pointer paging, and project-root-bound session switching. Project and session items use layered backgrounds with a full-height selection marker.
...
- **Diff presentation** — addition-only edits use a unified view even when split layout is selected, while additions and removals use dedicated low-saturation foreground and background colors in dark and light themes.
- **Tool document rendering** — compact thinking uses one content row between vertical padding and keeps the latest reasoning line beside its status, skips a redundant preview state for short content, and dispatch groups retain their own closing space after the final tool row; thinking and dispatch surfaces share recessive layered colors, group status animates only while work is active, each tool row gets vertical breathing room, and following prose keeps a stable gap; running rows animate foreground without relayout, edit metrics use semantic colors, turn summaries form a compact centered coda, document symbols and expanded content use consistent two-column gutters, and right-side controls keep a fixed alignment slot.
...
- **Filesystem tool output** — file reads, directory listings, searches, and errors share one live/replay projection with structured arguments, returned paths and matches, line numbers, syntax highlighting, bounded disclosure, and fullscreen detail.
...
- **Intent-first execution surfaces** — output blocks, workflow nodes, approvals, sub-agent panels, task windows, and active task leaves use localized call purposes as primary labels while retaining tool names, handles, arguments, request identities, and complete Bash/Terminal commands as auditable technical detail.
...
- **Flow discovery optional arguments** — `flow.search`, `flow.describe`, and `flow.spawn` schemas require non-empty cursors and source fingerprints, describe when fields must be omitted, and treat blank optional values as absent while retaining strict validation for malformed cursors and stale versions.
...
- **Indexed workflow projection** — live and restored workflow updates use append-stable node and tool paths, preserve exact flow-scoped tool completion, ignore duplicate structural frames, and report precise projection changes without recursively searching the complete workflow tree.
- **Incremental transcript layout** — TUI frames update transcript layout once from exact item revisions, use indexed viewport lookup, patch visible animation spans without rebuilding transcript or flow-panel projections, and retain only visible, overscanned, and bounded recent render rows instead of rescanning and caching the complete session.
...
- **Atomic MCP reloads** — enabled server namespaces retain their last valid tools until a complete replacement snapshot is ready; removed and disabled namespaces are discarded without clearing unrelated tools.
- **Live MCP tool refresh** — `notifications/tools/list_changed` rebuilds and atomically publishes changed tool snapshots, retains notifications emitted during connection startup, and keeps the last valid namespace when refresh fails.
- **Versioned flow execution** — `flow.spawn` can reject a stale discovery fingerprint, resolves installed flows through the configured Atman directory, and reports unknown or duplicate arguments instead of silently discarding them.
- **Explicit child handoffs** — spawned flows receive a scoped `handoff.parent` context record describing ownership, inheritance, capability boundaries, and the declared result without copying the delegated task text.
