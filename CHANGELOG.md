...
- **Project Hub navigation** — `Ctrl+L` opens a full-screen project workspace with a two-column board, fixed inspector, constrained session list, persistent item and page counts, keyboard and pointer paging, and project-root-bound session switching. Project and session items use layered backgrounds with a full-height selection marker.
