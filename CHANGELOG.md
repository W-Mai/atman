...
- **Tool document rendering** — compact thinking uses one content row between vertical padding and keeps the latest reasoning line beside its status; thinking and dispatch surfaces share recessive layered colors, group status animates only while work is active, each tool row gets vertical breathing room, and following prose keeps a stable gap; running rows animate foreground without relayout, edit metrics use semantic colors, turn summaries form a compact centered coda, and right-side controls keep a fixed alignment slot.
