...
Run `/spec <request>` in the TUI to explicitly start a synchronous requirements interview. The interview investigates project files, asks focused questions through multiline forms, stores research and design in configured project storage, publishes the design to the local preview, and waits for approval of that exact revision before returning. An explicitly requested design handled in the main agent uses the same revision-bound confirmation: the user response produces a session-local, single-use receipt, and only that receipt can approve or reject the materialized design.
...
Ctrl+P → Formula Rendering changes the same setting from the TUI; restart Atman to apply it.
...
The TUI imports a clipboard image with Cmd+V, Ctrl+V, or Alt+V. Pending images appear in a bar above the input and as stable `[image N]` references in the editor; deleting a reference removes that image, while Ctrl+U removes the latest image when no quote is pending. Images are copied into the session attachment store by content digest before the user turn is written. The effective reasoning depth appears at the input's top-right corner, and Ctrl+T selects the effort captured with the next submission. CLI and daemon runs accept repeatable `--image` arguments and the same `--reasoning` values. The selected value is invocation-local. `llm.call`, `llm.extract`, `llm.classify`, and `llm.generate_branches` consume it only when their own call explicitly passes `effort: env("effort")`; inheriting the invocation environment alone does not change a request. The managed agent's primary call opts in.
Flows can also use a fixed selection with `llm.call(reasoning: "high@pro")` or
`llm.call(reasoning_budget: 4096)`.

Drag across transcript text, including line-end space and short gaps between Markdown blocks, to open Copy, Rich Copy, Quote, and Cancel actions. Holding the pointer beyond the visible document edge scrolls the selection. Quote keeps the selected text separate from the editable prompt and sends it as a Markdown blockquote. Click the quote card or press F4 to expand it, then use PgUp/PgDn or the mouse wheel to scroll; click `[x]` or press Ctrl+U to remove it. When the terminal is too short for the card, the input border shows the pending quote count. Pending text tasks can be inserted into the current flow from the Next queue with `i` or its mouse action. The task appears in the transcript only when an LLM call includes it; otherwise it returns to Next when the turn ends.
...
Open **Manage Memory & Rules** from the TUI command palette (Ctrl+P). Action labels show their keys, including **[E]dit** and **[X] Archive**. In the editor, click to place the cursor, use Tab or Shift+Tab to change fields, Shift+Enter for a newline, Enter to save, or Esc to cancel. Search confessions and the rules loaded by the current agent, edit confession fields, and inspect revision history; original records remain intact. Archiving a confession requires confirmation, removes it from active memory and search, and retains its revision history. **Organize** generates category and related-record suggestions with the active model and shows progress or errors in the Suggestions pane; a fresh session uses the configured smart model. Review the suggestions, select individual rows or **Select All**, then apply the chosen categories. Rules are read-only in this panel; **Reload** refreshes their source files.
