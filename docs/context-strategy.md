...
Active root flow entries are keyed by their actual run identities. Tool calls using the `root` alias resolve it from the caller's execution identity, so one root cannot observe or control another root in the same session. External interjections pair a turn with its admitted run identity and follow the currently active root segment when that run redirects. Completed entries are removed while durable run identities and terminal events remain available for replay.
...
`Session::admit_turn_with_cancel` serializes source capture, full-window fork creation, the user message, and head selection as one short admission step. The daemon actor completes this step before starting executor bootstrap and passes the returned `ContextState` into the root invocation, so provider work never holds the admission lock and cannot silently switch to a newer default head. Multiple roots may run concurrently: each accepted submission owns its captured context, while the default head advances in admission order rather than worker completion order.
...
Scheduled compaction workers belong to the Session lifecycle. Session shutdown stops new background scheduling, cooperatively cancels provider and review waits, joins every active compaction, and only then closes the event writer. Cancellation removes the matching review and records a terminal compaction failure; it does not propagate into an independent flow-run cancellation token.
