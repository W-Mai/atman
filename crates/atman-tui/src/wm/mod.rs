...
impl WindowManager {
    ...
    pub fn dispatch_key(
        ...
        match content.handle_event(&WmEvent::Key(action.clone()), &mut ctx) {
            WmEventResult::Consumed(commands) => WmEventResult::Consumed(commands),
            WmEventResult::Ignored => WmEventResult::Ignored,
        }
    }
    ...
    pub fn dispatch_mouse(
        ...
        if !self.layers.modal_stack.is_empty() || app.mcp_add_form.is_some() || app.modal_notification.is_some() {
            return WmEventResult::Consumed(Vec::new());
        }
        match event.kind {
            MouseEventKind::ScrollUp => self.dispatch_scroll(3, true),
            MouseEventKind::ScrollDown => self.dispatch_scroll(3, false),
            MouseEventKind::ScrollLeft => self.dispatch_scroll_horizontal(3, true),
            MouseEventKind::ScrollRight => self.dispatch_scroll_horizontal(3, false),
            MouseEventKind::Down(MouseButton::Left) => {
                if let Some(panel) = self.hit_test_panel(event.column, event.row) {
                    self.focus(panel.id);
                    return WmEventResult::Consumed(Vec::new());
                }
                WmEventResult::Ignored
            }
            MouseEventKind::Moved => self.dispatch_hover(event.column, event.row),
            _ => WmEventResult::Ignored,
        }
    }

    pub fn dispatch_hover(&mut self, column: u16, row: u16) -> WmEventResult {
        self.interaction.hovered_panel_btn = self.hit_test_button(column, row);
        if self.interaction.hovered_panel_btn.is_some() || self.hit_test_panel(column, row).is_some() {
    ...
    pub fn dispatch_scroll(&mut self, delta: u16, page: bool) -> WmEventResult {
        if let Some(id) = self.focused_id()
            && let Some(panel) = self.panels.iter_mut().find(|panel| panel.id == id)
        {
            panel.scroll = if page {
                panel.scroll.saturating_add(delta)
            } else {
                panel.scroll.saturating_sub(delta)
            };
            return WmEventResult::Consumed(Vec::new());
        }
        WmEventResult::Ignored
    ...
    fn dispatch_scroll_horizontal(&mut self, delta: u16, left: bool) -> WmEventResult {
        if let Some(id) = self.focused_id()
            && let Some(panel) = self.panels.iter_mut().find(|panel| panel.id == id)
        {
            panel.h_scroll = if left {
                panel.h_scroll.saturating_sub(delta)
            } else {
                panel.h_scroll.saturating_add(delta)
            };
            return WmEventResult::Consumed(Vec::new());
        }
        WmEventResult::Ignored
    ...
    pub fn apply_commands(
        ...
        for command in commands {
            match command {
                ...
                WmCommand::ToggleMaximize(id) => self.toggle_maximize(id, app.maximized_canvas()),
                WmCommand::TermResize { handle, rows, cols } => {
                    if let Some(tx) = control_tx {
                        let _ = tx.send(crate::TuiControl::TermResize { handle, rows, cols });
                    }
                }
                WmCommand::OpenTaskPanel { handle, maximized, background } => {
                    app.open_task_panel(&handle, app.maximized_canvas());
                    if maximized {
                        if let Some(id) = self.panels.iter().find(|p| p.label == handle).map(|p| p.id) {
                            self.toggle_maximize(id, app.maximized_canvas());
                        }
                    }
                    let _ = background;
                }
                WmCommand::OpenPanel(spec) => {
                    let id = match spec.focus_policy {
                        FocusPolicy::Steal => self.open_with_size(&spec.label, spec.content_key, spec.open_policy, spec.content_kind, &spec.title, spec.canvas, spec.width, spec.height, spec.maximized),
                        FocusPolicy::Preserve => self.open_with_policy(FocusPolicy::Preserve, &spec.label, spec.content_key, spec.open_policy, spec.content_kind, &spec.title, spec.canvas, spec.width, spec.height, spec.maximized),
                    };
                    if let Some(panel) = self.panels.iter_mut().find(|panel| panel.id == id) {
                        panel.content = Some(spec.component.build());
                    }
                }
                WmCommand::PushToast { message, level, ttl, position } => app.push_toast(message, level, ttl, position),
            }
        }
    }

    pub fn render(&mut self, frame: &mut Frame, canvas: Rect, app: &mut crate::app::AppState) {
        self.interaction.last_hitmap = crate::wm::render(
            frame,
            canvas,
            self,
            &app.task_snapshots,
            &app.items,
            &app.activity_nodes,
            &self.interaction.hovered_panel_btn,
            &self.interaction.hovered_history_row,
            app.animation_frame,
            self.interaction.panel_close_armed_id.as_deref().map(|id| (id, !self.panel_close_arm_expired())),
            app.maximized_canvas(),
            !self.layers.modal_stack.is_empty(),
            &app.context.mcp_servers,
            &app.expanded_mcp_servers,
            app.mcp_selected,
            &self.interaction.hovered_mcp_row,
            &app.mcp_browser_state(),
            app.items_version,
            app.expanded_version,
        );
        self.layers.render_modals(frame, canvas, app, self.focused_id().unwrap_or(WindowId(0)));
        self.layers.render_blocking(frame, canvas, app);
        self.layers.render_toasts(frame, canvas, app);
    }
}
